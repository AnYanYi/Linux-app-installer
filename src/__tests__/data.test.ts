...
describe('Data Module', () => {
    ...
    describe('categories', () => {
        it('should have expected categories', () => {
            expect(categories).toContain('网页浏览器');
            expect(categories).toContain('通讯软件');
            expect(categories).toContain('开发：编辑器');
            expect(categories).toContain('游戏');
        });
    });

    describe('getAppsByCategory', () => {
        it('should return apps for a valid category', () => {
            const browsers = getAppsByCategory('网页浏览器');
            expect(browsers.length).toBeGreaterThan(0);
            browsers.forEach(app => {
                expect(app.category).toBe('网页浏览器');
            });
        });
