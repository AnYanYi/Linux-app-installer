...
import { distros, type DistroId, apps, type AppData } from '@/lib/data';
...
import { generateNixConfiguration, downloadNixConfig } from '@/lib/generateNixConfig';
...
export function CommandFooter({
    ...
    const handleDownloadNixConfig = useCallback(() => {
        ...
        const selectedPackages = Array.from(selectedApps)
            ...
            .filter((item): item is { app: AppData; pkg: string } => item !== null);

        const config = generateNixConfiguration({
        ...
        downloadNixConfig(config);
        analytics.scriptDownloaded('NixOS Configuration', selectedCount);
    }, [selectedCount, selectedDistro, selectedApps]);
                        ...
                        <div className="bg-[var(--bg-tertiary)] font-mono text-xs rounded-lg overflow-hidden border border-[var(--border-primary)]/40 shadow-2xl">
                            <div className="flex items-stretch">
                                ...
                                >
                                    ...
                                    <span className="font-bold whitespace-nowrap">预览</span>
                                    {selectedCount > 0 && (
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline whitespace-nowrap">下载</span>
                                </button>
                                ...
                                >
                                    ...
                                    <span className="hidden sm:inline whitespace-nowrap">{copied ? '已复制!' : '复制'}</span>
                                </button>
                            </div>
                        </div>
                    </div>
