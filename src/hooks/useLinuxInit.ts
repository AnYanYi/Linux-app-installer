...
import { decodeConfigFromUrl } from '@/lib/configManager';
...
export function useLinuxInit(): UseLinuxInitReturn {
    ...
    useEffect(() => {
        try {
            logger.debug('Loading user preferences', { source: 'localStorage/URL' });
            // First check for URL config parameter
            const urlParams = new URLSearchParams(window.location.search);
            const configParam = urlParams.get('config');
            
            let loadedDistro: DistroId | null = null;
            let loadedApps: string[] = [];
            
            if (configParam) {
                // Load from URL parameter
                const urlConfig = decodeConfigFromUrl(configParam);
            ...
            if (!loadedDistro) {
                ...
                if (savedApps) {
                    loadedApps = JSON.parse(savedApps) as string[];
                }
            }
            ...
            // Ignore errors
            logger.error('Error loading config', e as Error);
    ...
    useEffect(() => {
        ...
        try {
            ...
            logger.warn('Failed to save preferences to localStorage', { error: (e as Error).message });
        }
    ...
    const generatedCommand = useMemo(() => {
        perfMonitor.start('generateCommand');
        
        if (selectedApps.size === 0) {
            perfMonitor.end('generateCommand');
